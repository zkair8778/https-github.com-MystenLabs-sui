...
impl<T> IntentMessage<T> {
    ...
    /// Parse IntentMessage<TransactionData> from bytes.
    pub fn from_bytes(bytes: &[u8]) -> Result<IntentMessage<TransactionData>, anyhow::Error> {
        const INTENT_LENGTH: usize = 3;

        let intent: Intent = match bcs::from_bytes(
            bytes
                .get(0..INTENT_LENGTH)
                .ok_or_else(|| anyhow::anyhow!("Failed to parse Intent"))?,
        ) {
            Ok(intent) => intent,
