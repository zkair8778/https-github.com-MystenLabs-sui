...
use self::crypto::{bls12381, bulletproofs, ecdsa, ed25519, elliptic_curve, groth16, hmac};
...
pub fn all_natives(
    ...
    let sui_natives: &[(&str, &str, NativeFunction)] = &[
        ("ecdsa", "ecrecover", make_native!(ecdsa::ecrecover)),
        (
            "ecdsa",
            "decompress_pubkey",
            make_native!(ecdsa::decompress_pubkey),
        ),
        ("ecdsa", "keccak256", make_native!(ecdsa::keccak256)),
        (
            "ecdsa",
            "secp256k1_verify",
            make_native!(ecdsa::secp256k1_verify),
        ),
        (
            "dynamic_field",
            "hash_type_and_key",
