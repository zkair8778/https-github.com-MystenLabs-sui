...
#[tokio::test]
async fn macro_test_sallydb() {
    let primary_path = temp_dir();
    let tbls_primary = SallyDBExample::init(SallyDBOptions::RocksDB((
        primary_path.clone(),
    ...
    // Open in secondary mode
    let tbls_secondary = SallyDBExample::get_read_only_handle(SallyReadOnlyDBOptions::RocksDB((
        primary_path.clone(),
        None,
        None,
    )));
    ...
    // Check the counts
    assert_eq!(9, tbls_secondary.count_keys("col1").unwrap());
    assert_eq!(7, tbls_secondary.count_keys("col2").unwrap());

    // Test all entries
    let m = tbls_secondary.dump("col1", 100, 0).unwrap();
    for (k, v) in keys_vals_1 {
        assert_eq!(format!("\"{v}\""), *m.get(&format!("\"{k}\"")).unwrap());
    }

    let m = tbls_secondary.dump("col2", 100, 0).unwrap();
    for (k, v) in keys_vals_2 {
        assert_eq!(format!("\"{v}\""), *m.get(&k.to_string()).unwrap());
    }
    ...
    tbls_primary
        ...
        .expect("Failed to multi-insert");
    // New entries should be present in secondary
    assert_eq!(19, tbls_secondary.count_keys("col1").unwrap());

    // Test pagination
    let m = tbls_secondary.dump("col1", 2, 0).unwrap();
    assert_eq!(2, m.len());
    assert_eq!(format!("\"1\""), *m.get(&"\"1\"".to_string()).unwrap());
    assert_eq!(format!("\"2\""), *m.get(&"\"2\"".to_string()).unwrap());

    let m = tbls_secondary.dump("col1", 3, 2).unwrap();
    assert_eq!(3, m.len());
    assert_eq!(format!("\"7\""), *m.get(&"\"7\"".to_string()).unwrap());
    assert_eq!(format!("\"8\""), *m.get(&"\"8\"".to_string()).unwrap());
}
